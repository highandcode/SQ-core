...
const SQDialog = ({ className = '', closeButton = true, hideBackdrop = false, backDropClick = true, disableEscapeKeyDown, headerTag, transitionDir = 'up', classes: overrideClasses = {}, fullScreen = false, open = false, isLoading = false, title, content, children, onClose, actions = [], onAction }) => {
  ...
  return (
    <div className={`sq-dialog ${overrideClasses.root || ''}`}>
      <Dialog hideBackdrop={hideBackdrop} disableEscapeKeyDown={disableEscapeKeyDown} TransitionComponent={transitionOptions[transitionDir]} classes={{root: className, ...overrideClasses.dialog}} open={open} fullScreen={fullScreen} onClose={handleClose}>
        <DialogTitle>

...
const LaunchTimer = ({ eyebrow, header = '', subHeader = '', value = '', className = '' }) => {
  const HeaderProps = {
    eyebrow,
    header,
    subHeader
  ...
  const minutes = utils.number.preFix(
    new utils.datetime.DateTime(value).diffInMinutes(new utils.datetime.DateTime()) - (days * 24 * 60 + hours * 60)
  );
  const seconds = utils.number.preFix(
    new utils.datetime.DateTime(value).diffInSeconds(new utils.datetime.DateTime()) - (days * 24 * 60 * 60 + hours * 60 * 60 + minutes * 60)
  );

  return (
    <div className={`sq-launch-timer ${className}`}>
      <div className="sq-launch-timer__digits">
        <LabelDisplay label="Day" className="sq-launch-timer__days" value={days} />
        <LabelDisplay label="Hour" className="sq-launch-timer__hours" value={hours} />
        <LabelDisplay label="Minute" className="sq-launch-timer__minutes" value={minutes} />
        <LabelDisplay label="Second" className="sq-launch-timer__seconds" value={seconds} />
      </div>
    </div>
  );
};

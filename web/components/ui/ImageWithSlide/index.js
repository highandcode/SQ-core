...
const ImageWithSlide = ({ image = {}, align = 'left', className = '', classes = {}, eyebrow, header, headerTag = 'h2' }) => {
  const HTag = headerTag;
  const { cmpType: imageType = 'img', imageUrl, ...restImage } = image;
    ...
    <div className={`sq-image-with-slide ${className} sq-image-with-slide--${align}`}>
      <div className="sq-image-with-slide__cnt">
        ...
        <div className={`sq-image-with-slide__panel ${common.toStringBlank(classes.panel)}`}>
          <div className={`sq-image-with-slide__panel-inner  ${common.toStringBlank(classes.innerPanel)}`}>

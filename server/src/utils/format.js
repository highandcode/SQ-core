...
const formatters = {
  ...
  currency: (value, { input = false, ...options } = {}) => {
    ...
    if (!common.isNullOrUndefinedBlank(value) && !isNaN(value)) {
      ...
      let strFinal = '';
      if (input) {
        const len = value.toString().indexOf('.') > -1 ? value.toString().substr(value.indexOf('.') + 1).length : 0;
        strFinal = len >= 1 ? valNumeric.toFixed(len > decimals ? decimals : len) : valNumeric.toString();
      } else {
        strFinal = valNumeric.toFixed(decimals);
      }
      return plusMin + sign + strFinal.replace(replacer.regex, replacer.replace) + (input && hasDotAtEnd ? '.' : '');
    }
  ...
  currencyAbr: (value, options = {}) => {
    ...
    if (!common.isNullOrUndefinedBlank(value)) {
      return plusMin + sign + (valNumeric * 1).toFixed(decimals).replace(replacer.regex, replacer.replace) + (postfix ? ` ${postfix}` : '');
    }
    return '';
  },
  ...
  strFix: (value, { postfix, prefix } = {}, data) => {
    return (prefix || '') + (value || '') + (postfix || '');
  },
  percentage: (value, options = {}) => {
    const { sign = '%', fixed = 2 } = options;
    return (value * 1).toFixed(fixed) + sign;
  },
